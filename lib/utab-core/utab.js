    ...
    constructor (db_url) {
        ...
        function teams_organize (tournament_id, rs, {simple: simple=false, force: force=false}={}) {
            ...
            return Promise.all([that.teams.read(tournament_id), that.speakers.read(tournament_id), that.teams.results.read(tournament_id), that.speakers.results.read(tournament_id), that.draws.read(tournament_id), that.configs.readOne(tournament_id)]).then(function (vs) {
                ...
                if (!force) { rs.map(r => res.teams.precheck(raw_team_results, teams, r)) }
                if (simple) {
        ...
        this.draws.teams = {
            get: function(tournament_id, _for, {
                ...
                return Promise.all([that.configs.readOne(tournament_id), that.teams.read(tournament_id), that.teams.results.organize(tournament_id, rs, {simple: simple}), that.institutions.read(tournament_id)])
                    .then(function (vs) {
        ...
        this.draws.adjudicators = {
            get: function(tournament_id, _for, draw, {
                ...
                loggers.draws('debug', 'arguments are: '+JSON.stringify(arguments))
                let rs = by !== undefined ? by : _.range(1, _for)
                return Promise.all([that.configs.readOne(tournament_id), that.teams.read(tournament_id), that.adjudicators.read(tournament_id), that.institutions.read(tournament_id), that.teams.results.organize(tournament_id, rs, {simple: simple}), that.adjudicators.results.organize(tournament_id, rs)])
                    .then(function (vs) {
        ...
        this.draws.venues = {
            get: function(tournament_id, _for, draw, {by: by, simple: simple=false, force: force=false, shuffle: shuffle=false}) {
                ...
                return Promise.all([that.configs.readOne(tournament_id), that.teams.read(tournament_id), that.venues.read(tournament_id), that.teams.results.organize(tournament_id, rs, {simple: simple})])
                    .then(function (vs) {
