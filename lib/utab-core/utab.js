...
class Utab extends DBHandler {
    constructor (db_url) {
        ...
        function teams_organize (tournament_id, rs, {simple: simple=false, force: force=false}={}) {
            ...
            loggers.results('debug', 'arguments are: '+JSON.stringify(arguments))
            return Promise.all([that.teams.read(tournament_id), that.speakers.read(tournament_id), that.teams.results.read(tournament_id), that.speakers.results.read(tournament_id), that.configs.readOne(tournament_id)]).then(function (vs) {
        ...
        function adjudicators_organize (tournament_id, rs, {force: force=false}={}) {
            loggers.results('adjudicators.results.organize is called')
            loggers.results('debug', 'arguments are: '+JSON.stringify(arguments))
            return Promise.all([that.adjudicators.read(tournament_id), that.adjudicators.results.read(tournament_id), that.configs.readOne(tournament_id)]).then(function(vs) {
        ...
        function speakers_organize (tournament_id, rs, {force: force=false}={}) {
        ...
        this.draws.teams = {
            get: function(tournament_id, _for, {
                ...
                return Promise.all([that.configs.readOne(tournament_id), that.teams.read(tournament_id), that.teams.results.organize(tournament_id, rs, {simple: simple, force: force}), that.institutions.read(tournament_id)])
                    .then(function (vs) {
        ...
        this.draws.adjudicators = {
            get: function(tournament_id, _for, draw, {
                ...
                loggers.draws('debug', 'arguments are: '+JSON.stringify(arguments))
                let rs = by !== undefined ? by : _.range(1, _for)
                return Promise.all([that.configs.readOne(tournament_id), that.teams.read(tournament_id), that.adjudicators.read(tournament_id), that.institutions.read(tournament_id), that.teams.results.organize(tournament_id, rs, {force: force, simple: simple}), that.adjudicators.results.organize(tournament_id, rs, {force: force})])
                    .then(function (vs) {
        ...
        this.draws.venues = {
            get: function(tournament_id, _for, draw, {by: by, simple: simple=false, force: force=false, shuffle: shuffle=false}) {
                ...
                return Promise.all([that.configs.readOne(tournament_id), that.teams.read(tournament_id), that.venues.read(tournament_id), that.teams.results.organize(tournament_id, rs, {simple: simple, force: force})])
                    .then(function (vs) {
