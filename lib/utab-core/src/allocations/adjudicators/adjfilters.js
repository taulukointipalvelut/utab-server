...
function filter_by_past(adjudicator, g1, g2, {teams: teams, compiled_adjudicator_results: compiled_adjudicator_results, config: config, r: r}) {
    var g1_watched = math.count_common([g1.teams.gov, g1.teams.opp], sys.find_one(compiled_adjudicator_results, adjudicator.id).judged_teams)
    var g2_watched = math.count_common([g2.teams.gov, g2.teams.opp], sys.find_one(compiled_adjudicator_results, adjudicator.id).judged_teams)
...
function filter_by_institution(adjudicator, g1, g2, {teams: teams, compiled_adjudicator_results: compiled_adjudicator_results, config: config, r: r}) {
    var g1_institutions = Array.prototype.concat.apply([], [g1.teams.gov, g1.teams.opp].map(t => tools.find_and_access_detail(teams, t, r).institutions))
    var g2_institutions = Array.prototype.concat.apply([], [g2.teams.gov, g2.teams.opp].map(t => tools.find_and_access_detail(teams, t, r).institutions))
...
function filter_by_conflict(adjudicator, g1, g2, {teams: teams, compiled_adjudicator_results: compiled_adjudicator_results, config: config, r: r}) {
    var g1_conflict = math.count_common([g1.teams.gov, g1.teams.opp], tools.access_detail(adjudicator, r).conflicts)
    var g2_conflict = math.count_common([g2.teams.gov, g2.teams.opp], tools.access_detail(adjudicator, r).conflicts)
    if (g1_conflict > g2_conflict) {
        return 1
    } else if (g1_conflict < g2_conflict) {
        return -1
