...
function integrate_filter_functions(square, filter_functions, weights, dict) {
    loggers.silly_logger(integrate_filter_functions, arguments, 'draws', __filename)
    function f(a, b) {
        let a_val = 0
        let c = 0
        for (let func of filter_functions) {
            a_val += weights[c] * func(square, a, b, dict)
            c += 1
        }
...
function get_adjudicator_ranks_straight (r, allocation, teams, adjudicators, compiled_adjudicator_results, filter_functions, config) {
    ...
    for (var square of allocation) {
        let others = allocation.filter(other => square.id !== other.id)
        others.sort(integrate_filter_functions(square, filter_functions, weights, {teams: teams, compiled_adjudicator_results: compiled_adjudicator_results, config: config, r: r}))

        ranks[square.id] = others.map(o => o.id)
    }
...
function get_adjudicator_draw (r, draw, adjudicators, teams, compiled_team_results, compiled_adjudicator_results, nums, config, {filters: filters=['by_bubble', 'by_strength', 'by_num', 'by_num_chair', 'by_conflict', 'by_institution', 'by_past'], method: method='straight'}) {
    ...
    const ranks = get_ranks_methods[method](allocation, available_adjudicators, filter_functions_adj, { teams: available_teams, r, compiled_adjudicator_results, config })
