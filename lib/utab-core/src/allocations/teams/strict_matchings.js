...
function match(pools, pullup_method, config) {//pullup_method => fromtop, frombottom, random
    loggers.silly_logger(match, arguments, 'draws', __filename)
    let pools_cp = _.clone(pools)
    let matching_pools = []
    let pullup_func = pullup_funcs[pullup_method]

    for (let index = 0; index < pools_cp.length; index++) {
        let pool = pools_cp[index]
        if (pool.length === 0) { continue }
        matching_pools.push(pool)
        if (pool.length % 2 === 1) {
            let chosen = pullup_func(pools_cp[index+1], config)
            matching_pools[matching_pools.length-1].push(chosen)
            pools_cp[index+1] = pools_cp[index+1].filter(id => id !== chosen)
        }
    }
...
function strict_matching(teams, compiled_team_results, config, {pairing_method: pairing_method='random', pullup_method: pullup_method='fromtop', position_method: position_method='adjusted', avoid_conflict: avoid_conflict=true}) {
    ...
    wins.sort((a, b) => a > b ? 1 : -1)

    for (var win of wins) {
        var same_win_teams = team_ids.filter(id => sys.find_one(compiled_team_results, id).win === win)
        pools.push(same_win_teams)
    }

    let matching_pools = match(pools, pullup_method, config)
    let pre_matching = Array.prototype.concat.apply([], matching_pools.map(pool => pairing_funcs[pairing_method](pool, config, compiled_team_results)))
...
function pullup_func_fromtop(teams, config) {///TESTED///
    return teams[0]
}

function pullup_func_frombottom(teams, config) {///TESTED///
    let sorted_teams = teams.slice().reverse()
    return sorted_teams[0]
}

function pullup_func_random(teams, config) {
    let shuffled_teams = math.shuffle(teams, config.name)
    return shuffled_teams[0]
}
...
function pairing_func_fold(teams, config, compiled_team_results) {///TESTED///
    let matched = []
    let divided = divide_into(teams, 2)//config.style.team_num)
    for (let j = 1/*config.style.team_num-1*/; j >= 1/*config.style.team_num/2*/; j--) {
        divided[j].reverse()
    }
    for (let i = 0; i < teams.length/2/*team_num*/; i++) {
...
function pairing_func_slide(teams, config, compiled_team_results) {///TESTED///
    let matched = []
    let divided = divide_into(teams, 2)//config.style.team_num)
    for (let i = 0; i < teams.length/2/*config.style.team_num*/; i++) {
        matched.push(divided.map(div => div.filter((x, j) => j === i)[0]))
    }
    return matched
}
...
function pairing_func_sort(teams, config, compiled_team_results) {///TESTED///
    let matched = divide_into(teams, teams.length/2)//config.style.team_num)
    return matched
}

function pairing_func_adjusted(teams, config, compiled_team_results) {
    let all_cs = divide_comb(teams, 2)//config.style.team_num)
    all_divs = all_cs.map(c => divide_into(c, teams.length/2))//config.style.team_num))
