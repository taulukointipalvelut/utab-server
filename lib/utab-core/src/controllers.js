...
class DBHandler {//TESTED//
    constructor(db_url) {
        ...
        this.teams.results = new ResultsCollectionHandler(RawTeamResult)
        this.speakers.results = new ResultsCollectionHandler(RawSpeakerResult)
        this.adjudicators.results = new ResultsCollectionHandler(RawAdjudicatorResult)
    }
    destroy () {
        loggers.controllers('debug', 'Database was removed')
        this.conn.db.dropDatabase()
    }
    close() {
