...
function hash (val) {
    return parseInt(md5(val, Date.now()).slice(0, Number.MAX_SAFE_INTEGER.toString().length - 2), 16)
}
...
function pre_update(dict) {
    let new_dict = Object.assign({}, dict)
    delete new_dict.created
    delete new_dict.updated
    delete new_dict.version
    return new_dict
}
...
class DBHandler {//TESTED//
    constructor({ db_uri, db_tournaments_name }) {
        ...
        var conn = mongoose.createConnection().openUri(db_uri, { dbName: db_tournaments_name, useNewUrlParser: true, useUnifiedTopology: true })
        this.conn = conn
        ...
        this.conn.once('open', function() {
            loggers.controllers('connected to the database @ DBHandler of '+db_uri)
        })
    ...
    destroy (tournament_id) {
        ...
        return Promise.all(targets.map(target => this[target].delete(tournament_id)))
    }
    close() {
...
class _CollectionHandler {//TESTED// returns Promise object
    ...
    create(tournament_id, dict) {//TESTED//
        ...
        return model.save().then(arrange_doc).catch(() => {
            let identity = get_identity(this.identifiers, dict)
            loggers.controllers('error', 'AlreadyExists'+JSON.stringify(identity))
            throw new errors.AlreadyExists(identity)
        })
    }
    update(tournament_id, dict) {//TESTED//
        ...
        return M.findOneAndUpdate(identity, {$set: pre_update(dict), $inc: {version: 1}}, {new: true}).exec().then(function(doc) {
            if (doc === null) {
    ...
    deleteOne(tournament_id, dict) {//TESTED//
        loggers.controllers(this.Model.modelName+'.deleteOne is called')
        loggers.controllers('debug', 'arguments are: '+JSON.stringify(arguments))
        var M = this.Model
        var identity = get_identity(this.identifiers, { tournament_id, ...dict })
    ...
    delete(tournament_id, dict=undefined) {//TESTED//
        loggers.controllers(this.Model.modelName+'.delete is called')
        loggers.controllers('debug', 'arguments are: '+JSON.stringify(arguments))
        var M = this.Model

        return M.remove({ tournament_id, ...dict }).exec().then(function(doc) {
            return arrange_doc(doc)
        })
    }
...
class ConfigCollectionHandler extends _CollectionHandler {
    ...
    readOne(id) {//TESTED//
        loggers.controllers(this.Model.modelName+'.readOne is called')
