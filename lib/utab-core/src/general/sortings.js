...
function get_scores(adjudicator, adjudicator_results) {
    var scores = []

    //console.log(compiled_adjudicator_results, adjudicator.id)
    for (var detail of adjudicator_results[adjudicator.id].details) {
        scores.push(detail.score)
    }
    return scores
}

function evaluate_adjudicator (adjudicator, adjudicator_results, preev_weights) {
    var scores = get_scores(adjudicator, adjudicator_results)
    if (scores.length === 0) {
...
function allocation_comparer (compiled_team_results, a, b) {
    var a_win = sys.find_one(compiled_team_results, a.teams.gov).win + sys.find_one(compiled_team_results, a.teams.opp).win
    var b_win = sys.find_one(compiled_team_results, b.teams.gov).win + sys.find_one(compiled_team_results, b.teams.opp).win
    return a_win < b_win ? 1 : -1
}

function measure_slightness(ts, compiled_team_results) {
    let win_slightness = math.sd([ts.gov, ts.opp].map(id => sys.find_one(compiled_team_results, id).win))
    let sum_slightness = math.sd([ts.gov, ts.opp].map(id => sys.find_one(compiled_team_results, id).sum))
...
function sort_adjudicators_with_preev(adjudicators, adjudicator_results, preev_weights) {
    loggers.silly_logger(sort_adjudicators_with_preev, arguments, 'general', __filename)
    var sorted_adjudicators = [].concat(adjudicators)
    sorted_adjudicators.sort((a, b) => evaluate_adjudicator(a, adjudicator_results, preev_weights) < evaluate_adjudicator(a, adjudicator_results, preev_weights))
    return sorted_adjudicators
}
...
function sort_allocation (allocation, compiled_team_results, comparer=allocation_comparer) {
    ...
    var sorted_allocation = allocation.slice()
