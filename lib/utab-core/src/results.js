    ...
    for (let draw of draws) {
        for (let square of draw.allocation) {
            ...
            for (let id of teams) {
                ...
                summarized[id].past_sides.push(square.teams.gov === id ? 'gov' : 'opp')
                summarized[id].past_opponents = summarized[id].past_opponents.concat(teams.filter(id2 => id2 !== id))
            }
        }
    }
    return summarized
    ...
    for (let draw of draws) {
        for (let square of draw.allocation) {
            ...
            for (let id of adjudicators) {
                if (!summarized.hasOwnProperty(id)) { continue }
                summarized[id].judged_teams = summarized[id].judged_teams.concat([square.teams.gov, square.teams.opp])
                summarized[id].num_experienced += 1
                summarized[id].num_experienced_chair += square.chairs.includes(id) ? 1 : 0
            }
        }
    }
    return summarized
...
function compile_adjudicator_results (adjudicator_instances, raw_adjudicator_results, draws, rs) {//TESTED//
    ...
    for (var id of adjudicators) {
        let result = {
            ...
            average: math.average(_averages[id]),
            sd: math.sd(_averages[id]),
            judged_teams: summarized_draws[id].judged_teams,
            num_experienced: summarized_draws[id].num_experienced,
            num_experienced_chair: summarized_draws[id].num_experienced_chair,
...
function compile_team_results () {//TESTED//
    ...
    for (var id of teams) {
        var result = {
            ...
            details: _details[id],
            past_opponents: summarized_draws[id].past_opponents,
            past_sides: summarized_draws[id].past_sides,
