...
function summarize_adjudicator_results(adjudicator_instances, raw_adjudicator_results, r) {//TESTED//
    ...
    for (var id of adjudicators) {
        ...
        let is_chair = filtered_adjudicator_results[0].is_chair
        var comments = filtered_adjudicator_results.map(ar => ar.comment).filter(c => c !== '')
        var result = {r, id, score, judged_teams, is_chair, comments}
...
function compile_adjudicator_results (adjudicator_instances, raw_adjudicator_results, rs) {//TESTED//
    ...
    for (let id of adjudicators) {
        ...
        _details[id] = []
        _judged_teams[id] = []
        _num_experienced[id] = 0
        _num_experienced_chair[id] = 0
        _comments[id] = []
    }
    ...
    for (let r of rs) {
        ...
        for (let summarized_adjudicator_result of summarized_adjudicator_results) {
            ...
            _num_experienced[id] += 1
            _num_experienced_chair[id] += summarized_adjudicator_result.is_chair ? 1 : 0
        }
    }
    ...
    for (var id of adjudicators) {
        let result = {
            ...
            num_experienced: _num_experienced[id],
            num_experienced_chair: _num_experienced_chair[id],
